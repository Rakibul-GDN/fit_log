    ...
    <div className="mx-auto max-w-7xl px-4 py-8">
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-muted-foreground">Browse exercises or create custom ones</p>
        </div>
        <Link className="rounded-md bg-primary px-4 py-2 text-sm font-medium text-primary-foreground transition hover:bg-primary/90" href="/exercises/create">
          Add Custom Exercise
        </Link>
      </div>

      <ExerciseSearchFilter category={category} onCategoryChange={(v) => { setCategory(v); setPage(1); }} onSearchChange={(v) => { setSearch(v); setPage(1); }} search={search} />

      {isLoading && <div className="mt-6"><ExerciseListSkeleton /></div>}
      {error && <p className="mt-6 text-destructive">Failed to load exercises.</p>}

      {data?.data && data.data.length === 0 && (
        <div className="mt-6 rounded-lg border border-dashed border-border p-8 text-center">
          <p className="text-lg text-muted-foreground">No exercises found</p>
          <Link className="mt-4 inline-block rounded-md bg-primary px-4 py-2 text-sm font-medium text-primary-foreground transition hover:bg-primary/90" href="/exercises/create">
